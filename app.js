...
(function() {
    ...
    function loadSample() { state.questions = [{ id: generateId(), text: "What is the capital of France?", type: "single", options: [{ text: "London", isCorrect: false }, { text: "Berlin", isCorrect: false }, { text: "Paris", isCorrect: true }, { text: "Madrid", isCorrect: false }] }, { id: generateId(), text: "Select programming languages", type: "multiple", options: [{ text: "Python", isCorrect: true }, { text: "HTML", isCorrect: false }, { text: "JavaScript", isCorrect: true }, { text: "CSS", isCorrect: false }] }, { id: generateId(), text: "What is a database?", type: "open", correctAnswers: ["Structured data", "Organized collection"], caseSensitive: false }]; state.participants = []; state.scoreRecords = []; state.currentSlideIndex = 0; renderQuestionsList(); renderSlideQuiz(); renderParticipantsSidebar(); renderScoreboard(); updateDatalist(); showMessage('Sample loaded'); }
    function exportScoresCSV() { if (!state.scoreRecords.length) { showMessage('No scores', true); return; } let csv = "Participant,Question,Points,Notes,Date\n" + state.scoreRecords.map(r => '"' + r.participantName + '","' + r.questionText.replace(/"/g, '""') + '",' + r.pointsEarned + ',"' + r.notes + '","' + r.timestamp + '"').join('\n'), b = new Blob([csv], { type: 'text/csv' }), a = document.createElement('a'); a.href = URL.createObjectURL(b); a.download = 'scores_' + Date.now() + '.csv'; a.click(); URL.revokeObjectURL(a.href); }
    ...
    function exportToPDF() { let w = window.open('', '_blank'); if (!w) { showMessage('Popup blocked', true); return; } let h = '<html><head><title>Report</title><style>body{font-family:sans-serif;padding:20px;}table{border-collapse:collapse;width:100%}th,td{border:1px solid #ddd;padding:8px}</style></head><body><h1>Quiz Report</h1><h2>Questions</h2>' + state.questions.map((q, i) => '<div><strong>' + (i + 1) + '.</strong> ' + escapeHtml(q.text) + ' (' + q.type + ')</div>').join('') + '<h2>Scores</h2><table><tr><th>Participant</th><th>Question</th><th>Points</th></tr>' + state.scoreRecords.map(r => '<tr><td>' + escapeHtml(r.participantName) + '</td><td>' + escapeHtml(r.questionText.substring(0, 50)) + '</td><td>' + r.pointsEarned + '</td></tr>').join('') + '</table></body></html>'; w.document.write(h); w.document.close(); w.print(); }
    ...
    function initEventListeners() {
        ...
        document.getElementById('sampleDataBtn').addEventListener('click', loadSample);
